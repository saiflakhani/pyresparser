...
def extract_colleges(nlp_text, noun_chunks, colleges_file=None):
    '''
    ...
    if not colleges_file:
        data = pd.read_csv(
            os.path.join(os.path.dirname(__file__), 'schools.csv'),
        )
        # school_data_df = data.T.reset_index()
        # school_data_df.columns = ['schools']
        # school_data_df.columns = ['schools']
        

    else:
        data = pd.read_csv(colleges_file)
        print(data)
    schools = list(data.columns.values)
    ...
    for token in tokens:
        if token.lower() in schools:
            schoolset.append(token)
    ...
    # school_data_df['combined'] = combined
    # school_data_df = school_data_df[school_data_df['combined'].str.contains(str(school_data_df['schools']))]
    # print(school_data_df)

    ## TODO OPTIMIZE
    for school in schools:
        if school.lower() in combined:
            schoolset.append(school)

    return [i.capitalize() for i in set([i.lower() for i in schoolset])]
    # return ["Meow", "Bhow"]
